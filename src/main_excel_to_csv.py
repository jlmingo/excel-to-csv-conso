...
def main():
    ...
    trigger=True
    while trigger==True:
        erase_csv = input("Do you want to remove CSV files? ()").lower()
    ...
    xlsx_to_csv(xlsx_path, csv_path, dtypes_sap)
    ...
    for f in os.listdir(csv_path):
        ...
        df = pd.read_csv(path_file, dtypes_sap)
        ...
        with open(check_file, "a") as txt:
            ...
            elif file_name != posicion_liquidez:
                if len(posicion_liquidez) > 0:
